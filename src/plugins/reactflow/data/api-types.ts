...
const nodeType: ApiEntry = {
  ...
  props: [
    ...
    { name: "measured", type: "{ width?: number; height?: number }", description: "Read-only output - dimensions measured by React Flow after layout. Do not write to this field." },
    { name: "width", type: "number", description: "Writable fixed width. Set to pin the node at an explicit size; overrides measured.width." },
    { name: "height", type: "number", description: "Writable fixed height. Set to pin the node at an explicit size; overrides measured.height." },
    { name: "initialWidth", type: "number", description: "Initial width hint for SSR and dynamic layout before measurement." },
    { name: "initialHeight", type: "number", description: "Initial height hint for SSR and dynamic layout before measurement." },
    { name: "handles", type: "NodeHandle[]", description: "Explicit handle definitions for this node." },
    { name: "ariaLabel", type: "string", description: "Accessible label for the node element." },
    { name: "resizing", type: "boolean", description: "Whether the node is currently being resized (set by React Flow internally)." },
    { name: "focusable", type: "boolean", description: "Override global nodesFocusable setting for this node." },
  ],
  ...
  tips: [
    "width/height (writable) set a fixed size; measured.width/measured.height (read-only) are the dimensions React Flow computed after DOM measurement - do not mix them up.",
    "v12: measured dimensions are at node.measured.width, not node.width.",
    "Use initialWidth/initialHeight for SSR scenarios so edges are positioned correctly before hydration.",
...
const reactFlowInstanceType: ApiEntry = {
  ...
  props: [
    ...
    { name: "updateEdgeData()", type: "(id, data) => void", description: "Update an edge's data object. Symmetric counterpart to updateNodeData." },
